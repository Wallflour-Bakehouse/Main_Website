...
import { faAngleDown, faAngleUp, faCalendarDay, faMessage, faMinus, faShoppingBag, faUtensils } from '@fortawesome/free-solid-svg-icons'
import {url} from '../../url'
                ...
                <div className="cart container mb-5">
                    ...
                    {cart.cartItems.map(item=>
                        <div className="row cart_item pt-3 pb-3" key={item._id}>
                            ...
                            <div className="col-3 col-md-2 col-lg-2 total">₹ {(item.product.price-(item.product.price*item.product.discount*0.01))*item.quantity}</div>
                            <div className="remove_item" onClick={()=>deleteProductFromCart(item.product._id, item.quantity)}><FontAwesomeIcon icon={faMinus} /></div>
                        </div>
                    )}
                    <div className="row">
